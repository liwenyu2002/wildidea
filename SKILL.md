...
`输入 -> 类型判断 -> 抽槽位 -> 隔离常规答案 -> 源域/外域机制冻结 -> 映射到用户领域 -> 联网查重过滤 -> 淘汰则重抽 -> 得到10条 -> 独立判官评映射质量(6维度0-10分，Structural Depth<5淘汰) -> 生成HTML -> 校验HTML -> 返回路径`
...
- **映射结构深度**：全部 10 条候选生成完毕后，由**独立判官**逐条评分（0-10 分），不得由生成者自评。判官 prompt 对齐 Shen et al. 2026 (arXiv:2605.11258) G.6 Analogy Judge Prompt。
  - 输入给判官：用户问题、源域名称、目标域名称、对象映射（源域中的 X → 目标域中的 Y）、共享关系（源域中 X 和 Y 的关系 → 目标域中对应物的关系）
  - 评分维度（0-10）：
  ...
  - Structural Depth 低于 5 分的候选必须重做映射或淘汰。最终 10 条的平均 Structural Depth 应 ≥ 6。
  - **判官模型选择**（按优先级）：
    1. `anthropic/claude-sonnet-4.5`（论文原版，temp=0）— 首选
    2. `deepseek/deepseek-r1`（免费/低成本）— 降级方案。注意：DeepSeek-R1 评分偏高（实测 SD 平均 9.0 vs Claude 的 7.8），使用时 Structural Depth 淘汰阈值应上调至 7，平均阈值上调至 8
    3. 任意其他 LLM — 用户自行校准阈值
  - 如果用户没有任何付费 API，可以用当前正在运行 WildIdea 的同一个模型当判官，但必须在统计行注明"判官模型=生成模型，评分可能 inflation"。

...
随机组词必须按 `references/search-integration.md` 搜索真实语境；没有真实规则、边界、阈值或事件就重抽。
...
5. 运行校验：`python scripts/validate_poster.py outputs/<topic>.html --forbid-proto-term <用户领域术语...>`。
   - **默认就要带 `--forbid-proto-term`**：从用户输入里抽取任务名、数据类型、对象、指标、常规方法名作为禁词（例如 EEG 任务就传 `EEG 脑电 被试 域适应 准确率`），用来卡住 `.proto` 泄露用户领域术语——这是去锚点纪律的硬执行，不是可选项。
   - 校验脚本除此之外还查：占位符、卡片数、`.source` 不能是槽位名、`.slot` 必须带 D1–D6、卡片不得残留模板样例文字、文字溢出防护。
   - 精简/极端/一杀模式卡片不足 10 张时加 `--cards N`。
6. 最终答复给出本地路径和 `file:///` 地址。

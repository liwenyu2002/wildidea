...
`输入 -> 类型判断 -> 抽槽位 -> 隔离常规答案 -> 源域/外域机制冻结 -> 映射到用户领域 -> 联网查重过滤 -> 淘汰则重抽 -> 得到10条 -> 独立判官评映射结构深度(低于3分重做或淘汰) -> 生成HTML -> 校验HTML -> 返回路径`
...
- **映射结构深度**：全部 10 条候选生成完毕后，由**独立判官**逐条评分（1-5 分），不得由生成者自评。判官只看到两样东西：源域原型（proto）和映射描述（desc），不知道来源域名称。评分标准：
  ...
  - **判官 prompt**：`你是独立的映射质量评审员。以下是源域机制的冻结描述和它被映射到目标领域的结果。请评估映射的结构深度（1-5分）：1=只借了名称或隐喻；2=转移了部分流程但丢失关键约束；3=转移了主流程和部分约束；4=转移了完整因果链、约束和边界条件；5=转移了数学/逻辑结构，可推导新预测。源域原型：{proto} 映射结果：{desc} 输出JSON：{"depth": <1-5>, "reasoning": "..."}`

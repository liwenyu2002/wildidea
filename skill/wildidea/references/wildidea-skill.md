...
name: wildidea
version: "1.4"
description: "WildIdea v1.4：统一使用源/目标隔离建图、结构对齐、独立判官、证据红队、蓝队存活/修复档案、目标领域查重、整批多样性与 HTML 校验。"
---

# WildIdea Skill v1.4 Full Spec
...
`输入问题与常见解法 -> 类型判断 -> problem_card（五键） -> 目标问题关系图 -> 按模式抽 9 个槽位 -> 源机制隔离建图 -> 结构对齐 -> 从对齐写卡 -> 基础校验 -> 独立判官 -> 证据红队 -> 蓝队记录存活理由/弱点/致命攻击 -> 定向修复或换结构角度（最多 2 次） -> 可证伪候选推论 -> 目标领域查重 -> 形成通过/未达标保底/失败结果 -> Vendi 与解法形式多样性检查 -> 生成并校验 HTML/search sidecar -> problem_card 五键覆盖盘点 -> 返回路径`
...
7. 每张最终可见卡必须形成红蓝对抗档案：红队尝试用反向映射、裸测退化和目标领域撞车证据杀死卡片；蓝队记录卡片为何存活、仍有哪些弱点，或把致命/可修复证据注入下一次生成。完整角色与证据格式见 `references/red-blue-workflow.md`。
...
- 每张最终可见卡都有 `survived / weaknesses / fatal / judge_dims` 红蓝档案；致命攻击只在附有反向映射、裸测文本、引用或直接实现证据时成立；
- 源机制图由看不到目标问题的隔离角色生成，候选卡从已接受的结构对齐写出，不允许卡片作者临时编造关系对；
...
- `desc`：落地方案。开头先声明本卡主攻 `problem_card` 五键中的哪一键，不设固定字数、句数或段落上限；按问题复杂度写清所攻差距、引用的背景事实与硬约束、参与者和资源、分阶段动作、触发条件、可观察指标及难以被快速复制的原因。不得用重复整段背景来冒充针对性。
...
标准模式中，生成者不能自评。独立判官只接收用户问题、`problem_card`、源域、目标域、已接受的结构对齐与候选文本，不接收生成过程；除了给 6 维打分，还要核对候选是否真的命中它 `desc` 声明主攻的那个 `problem_card` 键。判官之后还必须进入独立红队；判官高分不能覆盖带证据的致命攻击。
...
1. 目标问题先建图；每个源锚点由看不到目标问题的角色独立建图，再由 aligner 对齐两图。
2. 卡片作者只从对齐关系生成候选并做基础校验；失败则记录具体规则。
3. 独立判官评分，同时核对系统性与 `problem_card` 对应关系。
4. 红队执行结构杀手、裸测和可用时的陈旧性搜索；致命证据直接淘汰或进入修复，不能被平均分抵消。
5. 蓝队记录存活理由、残余弱点与最强/最弱判官维度。失败证据按三段式进入下一次尝试：先定向修复，连续同因再换同一锚点的结构角度。
6. 达到质量与红蓝门槛后，再做联网审查；查重开关生效时还需过 Target Field Novelty Check。
7. 联网确认直接同机制实现时，本次淘汰；搜索无法判断时，标准模式不得将其标为通过。
8. 未过分数门槛但未被证据证明无效的候选可进入保底候选集；有致命结构反例、虚假来源或直接撞车的候选不得保底。
9. 最多三次；若曾进入判官，三次结束后按四项均分选择可搜索确认的最高保底；若三次都未通过质量门、从未进入判官，改选违规规则最少且结构仍有效的一版；两种保底都取不出结果时该槽位失败。
